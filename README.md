...
- Shopify SEO review: Project Detail can store a draft-backed product SEO snapshot for Shopify storefronts, seed a plug-and-play `sinternetcult.com` review set, import product JSON, select products, and queue product-level SEO review batches without live Shopify writes
...
- **Shopify SEO product batches** — Project Detail -> Shopify SEO stores review snapshots as `shopify_seo_review` drafts, then queues selected products as product-scoped LLM tasks. Batch context is stage-only and explicitly blocks live Shopify writes until a future approval/apply path exists.
