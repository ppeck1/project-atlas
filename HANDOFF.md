...
**Remote portfolio inventory disclosure is tiered and activation-gated.** Policy v2 separates a compact `list_projects` inventory tier from the detail tier used by status, workload, and planning context. Inventory capacity is 256, detail capacity is 64, list pages are 64 rows, and valid v1 rows remain compatible as both capabilities. Settings previews exact candidate labels and aliases locally, escapes unsafe titles, reports bounded validation causes, tracks source-title drift with optional local-only SHA-256 baselines, and never serializes local IDs or unenrolled labels. The controlled live-data candidate currently has 48 immediately safe inventory labels out of 49 eligible projects; one title needs an operator-approved remote replacement before the policy can cover the full portfolio. The production v1 policy, gateway on port 4874, and existing tunnel remain unchanged until explicit approval.
