...
**Project AI summaries are opt-in.** The structured project summary pipeline is gated through Settings -> AI Summaries. Manual Project Detail summaries can be enabled without enabling bulk refresh; bulk refresh remains a separate operator gate. Summary generation can include linked Library documents by default, can use a summary-specific installed Ollama model, previews the ranked evidence packet in Project Detail, stores the model input in draft `input_json`, records correlated summary provenance events, and fails closed when schema, people, document, or generic-action validation fails.
