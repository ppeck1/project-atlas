...
**Human-in-the-loop AI.** Every Ollama response is shown to the user in a review dialog before it is saved. Nothing is auto-applied. `OllamaResult.isSuccess == false` means the server was unavailable, returned empty, or returned transport/timeout text; the UI shows a SnackBar/status and stops. Structured project summaries also fail closed through `ProjectSummaryOutcome.validationIssues` when schema, claim, ownership, document ID, or unsupported generic-action validation fails.
...
**Project AI summaries are opt-in.** The structured project summary pipeline is gated through Settings -> AI Summaries. Manual Project Detail summaries can be enabled without enabling bulk refresh; bulk refresh remains a separate operator gate. Summary generation can include project-linked Library documents by default, can use a summary-specific installed Ollama model, previews a categorized ranked evidence packet with packet warnings in Project Detail, stores the model input in draft `input_json`, records correlated summary provenance events, exposes deterministic evaluation JSON for contract tests, and fails closed when schema, people, document, or generic-action validation fails.

**Project change summaries are project-persistent.** Project Detail -> Change Log reads normalized project event rows newest-first by default and exposes newest/oldest sort controls. The Summarize changes action is owned by `AppState.startProjectChangeSummary()`, so navigation away from Project Detail does not cancel the request. Successful summaries are saved as `kind='project_change_summary'` drafts with `project_change_summary_draft_input_v1` input JSON. Timeout or transport failures are tracked as run errors and are not displayed as the latest saved summary.

**Shopify SEO review is draft-backed and stage-only.** Project Detail -> Shopify SEO stores product review snapshots as `kind='shopify_seo_review'` drafts, accepts v1/v2 Shopify-like JSON without requiring an Admin API token, analyzes deterministic SEO issues through `ShopifySeoAnalyzer`, generates proposal seeds from imported product data only, and queues one product per LLM batch with `writePolicy='stage_only_until_user_approved'`. The workflow exports JSON/CSV/Markdown review packets and intentionally has no live Shopify write/apply button.
...
**Agent boundary is proposal-first, with an explicit LLM queue.** `AtlasAgentService` in `lib/services/atlas_agent_service.dart` is the desktop-side adapter for local MCP clients, the read-only remote gateway, and local LLM harnesses. It exposes read-heavy project operations (`listProjects`, `getProjectStatus`, `getProjectBrief`, freshness/status preflights, local refresh preview, git visibility inspection, enrichment run history), persisted LLM queue lifecycle methods, operator-owned queue edit/cancel/requeue methods, and proposal-first write requests saved as drafts with `kind='atlas_agent_proposal'`. `lib/mcp/atlas_mcp_server.dart` maps MCP-style tool calls to that service for reads, Atlas-only enrichment runs, queue enqueue/list/get/claim/complete/fail, read-only Workboard planning views, and proposal creation; approval/rejection and queue edit/cancel controls are intentionally not exposed to agents. LLM queue tasks can carry linked project media; MCP `get_llm_task` returns the task plus attached media metadata for harness context. Library has an Agent Proposals filter where pending proposals can be approved or rejected. Approval applies supported status, task, and manifest metadata/tag changes through `AppState`; validation approvals are logged; handoff approvals create `project_handoff` drafts. Queue completion can attach a `handoff_record` proposal draft via `proposalBody`, but it does not directly mutate project state. The Workboard MCP tools (`atlas.workload_snapshot`, `atlas.project_workload`, `atlas.suggest_next_work`, and `atlas.work_item_context_bundle`) do not claim tasks, complete work, enqueue work, run harness jobs, or mutate queue/project state. The ChatGPT remote MCP profile is still narrower: exactly `list_projects`, `get_project_status`, `atlas.workload_snapshot`, and `atlas.project_planning_context`, with ignored policy-approved aliases and fresh projected DTOs. Settings -> Integrations includes a local-only disclosure preview for that profile; it reads ignored config/policy/audit metadata and loopback gateway metadata, never starts the gateway/tunnel, and reports active executable identity as unverified until process attestation exists. The service does not delete projects, overwrite manifests, push/fetch Git, or mutate discovered repositories; human review remains the approval boundary.

**Remote portfolio inventory disclosure is tiered and active.** Policy v2 separates a compact `list_projects` inventory tier from the detail tier used by status, workload, and planning context. Inventory capacity is 256, detail capacity is 64, list pages are 64 rows, and valid v1 rows remain compatible as both capabilities. Settings previews exact candidate labels and aliases locally, escapes unsafe titles, reports bounded validation causes, tracks source-title drift with optional local-only SHA-256 baselines, and never serializes local IDs or unenrolled labels. The production policy on port 4874 contains 49 operator-approved inventory entries: 2 detail-approved and 47 inventory-only. One conservative identifier-shaped source title uses an operator-approved remote label and alias. Authenticated connector readback verified the complete 49-project inventory, detail isolation, and the retained tunnel.

**Portfolio signals are calibrated separately from freshness debt.** WO-PSC-1 adds a bounded `signals` object to the projected inventory/status/planning contracts. `planningActionRequired` covers lifecycle/workload blockers; `dataRefreshRequired` covers evidence maintenance. `needsAttention` remains a compatibility alias for planning action only, stale and unknown stay distinct, and severity/reason classes are fixed enums. Workload counts name `workItems` and `llmQueueItems` separately, and workload/planning packets emit a valid non-null UTC `generatedAt`. Production activation from source commit `24054b586ffbe0893d3381ef21dc0b7c94d9bce7` passed authenticated connector readback: inventory v3 returned 49 rows (2 detail/47 inventory-only), 28 planning-action and 48 refresh signals; status/workload/planning returned v2, 15 work items and 16 LLM queue items, and valid packet timestamps. The tracked reason matrix is read-only evidence; this work order does not bulk-clear freshness or lifecycle state.
...
| `drafts` | AI-generated drafts, including project summaries, project change summaries, agent proposals, and human-reviewed outputs |
...
- Actions: today summary, email draft, task extract, work item analysis, opt-in structured project summary, and Project Detail change-log summary. Project summaries are controlled in Settings -> AI Summaries and can use the global Ollama model or a summary-specific installed model. Change-log summaries save only on successful output and use compact prompt evidence.
- Timeout: 300 seconds (5-minute) by default; Project Change Log summaries use a 12-minute timeout because local Ollama change-log evidence packets can be slower.
