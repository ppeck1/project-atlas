...
**Human-in-the-loop AI.** Every Ollama response is shown to the user in a review dialog before it is saved. Nothing is auto-applied. `OllamaResult.isSuccess == false` means the server was unavailable or returned empty; the UI shows a SnackBar and stops. Structured project summaries also fail closed through `ProjectSummaryOutcome.validationIssues` when schema, claim, ownership, document ID, or unsupported generic-action validation fails.
